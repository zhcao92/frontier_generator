#!/usr/bin/env python3
"""Coverage-aware DETR: predicts frontier position + gain score.

Trained from the coverage dataset (step_{fid}.json files) generated by the
geometric pipeline.  Adds a score regression head on top of the standard
DETR position + confidence architecture.
    ...
    for fi, fid in enumerate(fids):
        ...
        for wp_id in wids:
            ...
            if cands:
                ...
                cand_scores = np.array([c['gain_m2'] for c in cands],
                                       dtype=np.float32)
...
def _build_coverage_detr():
    ...
    class CoverageDETR(nn.Module):
        def __init__(self, d_model=256, nhead=8, num_encoder_layers=3,
            ...
            # Score head: predicts log1p(gain_m2)
        ...
        def forward(self, x):
            ...
            Returns:
                ...
                pred_score: (B, Q) predicted log1p(gain_m2)
            """
            B = x.size(0)
...
def hungarian_loss_with_score(pred_xy, pred_conf, pred_score,
    ...
    """Compute Hungarian matching loss including score regression.
    ...
    Args:
        ...
        target_score : (B, Q) padded target gain_m2 (raw, not log1p)
    ...
    for b in range(B):
        ...
        # Matched pairs: L1 position loss
        ...
        # Score loss on matched pairs: SmoothL1(pred, log1p(gain_m2))
        gt_score_log = torch.log1p(target_score[b, :nt][col_ind])
        total_score = total_score + F.smooth_l1_loss(
            pred_score[b][row_ind], gt_score_log, reduction='sum')
    ...
    loss_score = lambda_score * total_score / n_total
...
# ── Metrics (extends existing with score MAE) ──────────────────────────
def compute_coverage_metrics(pred_xy, pred_conf, pred_score,
                             target_xy, target_score, n_targets,
                             conf_thresh=0.5,
                             dist_thresholds_m=(0.5, 1.0)):
    """Compute AP metrics plus score MAE (m²) on matched pairs."""
    torch, _, _ = _import_torch()
    from scipy.optimize import linear_sum_assignment
    ...
    for b in range(B):
        ...
        # Hungarian for mean L1 and score MAE
        ...
        if cost_np.shape[0] > 0 and cost_np.shape[1] > 0:
            ...
            for r, c in zip(ri, ci):
                ...
                l1_count += 1
                # Score MAE in m²: expm1(pred) vs gt gain_m2
                p_score = float(torch.expm1(pred_score[b][mask][r]).item())
                g_score = float(target_score[b, c].item())
                score_ae_sum += abs(p_score - g_score)
                score_ae_count += 1
    ...
    metrics['score_mae_m2'] = score_ae_sum / max(score_ae_count, 1)
...
def cmd_train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"Epoch {epoch:3d}/{args.epochs}  "
              ...
              f"sMAE={vm['score_mae_m2']:.1f}m²  lr={lr:.2e}")
...
def cmd_predict(args):
    ...
    all_pred_world = []    # [(x, y, conf, score_m2), ...]
    all_gt_world = []      # [(x, y, gain_m2), ...]

    print(f"Frontier {fid}: {len(wids)} WPs")
    if gt_candidates is not None:
        print(f"  GT candidates: {len(gt_candidates)}")
    print(f"{'WP':>6}  {'#GT':>4}  {'#Pred':>5}  Predictions (row,col,conf,score_m²)")
    print("-" * 80)
    ...
    for wp_idx, wp_id in enumerate(wids):
        ...
        for i, xy in enumerate(gt_xy):
            g = gt_gains[i] if i < len(gt_gains) else 0.0
            all_gt_world.append([xy[0], xy[1], g])

        # Console summary
        pred_str = '  '.join(
            f'({r:.2f},{c:.2f},{conf:.2f},{sc:.0f})'
            for (r, c), conf, sc in zip(preds_bev, confs, scores_m2)
        ...
        # Panel 1 – input + GT candidates (sized by gain_m2)
        axes[1].imshow(vis)
        ...
        if len(gt_bev):
            ...
            gg = gt_gains[gt_valid] if len(gt_gains) else np.array([])
            # Size by gain: min 40, scale up
            gt_sizes = 40 + np.clip(gg, 0, 200) * 0.6 if len(gg) else 40
        ...
        # Panel 2 – input + predictions (sized by score, annotated)
        axes[2].imshow(vis)
        axes[2].scatter(WC, WC, c='orange', s=150, marker='*',
                        edgecolors='white', linewidths=1.5, zorder=5)
        if len(preds_bev):
            pv = preds_bev * GRID_SIZE
            # Size by predicted score
            pred_sizes = 40 + np.clip(scores_m2, 0, 200) * 0.6
    ...
    # GT candidates (sized by gain_m2)
    if all_gt_world:
        gt_arr = np.array(all_gt_world)  # (N, 3): x, y, gain_m2
        # Deduplicate by position
        _, uniq_idx = np.unique(np.round(gt_arr[:, :2], 3),
                                axis=0, return_index=True)
        gt_uniq = gt_arr[uniq_idx]
        gt_sizes = 20 + np.clip(gt_uniq[:, 2], 0, 200) * 0.4
    ...
    # Predicted candidates (sized by score, alpha by confidence)
    if all_pred_world:
        pred_arr = np.array(all_pred_world)  # (N, 4): x, y, conf, score_m2
        pred_sizes = 20 + np.clip(pred_arr[:, 3], 0, 200) * 0.4
